...
export function buildPlannerIntentPromptParts(input: PlannerInput): { systemPrompt: string; userPrompt: string } {
  ...
  return {
    systemPrompt: [
      ...
      "如果用户是在描述一本完整新书或短故事的生产任务，请使用 produce_novel，并尽量提取 contentForm、description、targetChapterCount、genre、worldType、styleTone、projectMode、pacePreference、narrativePov、emotionIntensity、aiFreedom、defaultChapterLength、targetTotalWordCount。",
      ...
      "contentForm 只能是 novel 或 short_story；用户说短故事、短篇、中短篇时用 short_story；emotionIntensity 和 aiFreedom 只能是 low、medium、high；defaultChapterLength 是 500 到 10000 的整数。",
