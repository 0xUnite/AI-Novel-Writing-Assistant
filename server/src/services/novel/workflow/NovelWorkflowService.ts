...
export class NovelWorkflowService {
  ...
  private async shouldHideStaleAutoDirectorTask(
    novelId: string,
    row: NonNullable<Awaited<ReturnType<typeof prisma.novelWorkflowTask.findUnique>>>,
  ) {
    ...
    if (seedPayload?.directorSession) {
      return false;
    }
    if (row.checkpointType === "front10_ready" || row.checkpointType === "chapter_batch_ready") {
      return false;
    }

    const [novel, characterCount, chapterCount, volumeCount] = await Promise.all([
      ...
      prisma.volumePlan.count({ where: { novelId } }),
    ]);

    const resourceReadyScore = novel?.resourceReadyScore ?? 0;
    const hasRecoveredProductionAssets = resourceReadyScore >= 80
      || (characterCount >= 3 && chapterCount >= 10 && volumeCount >= 1);
    const missingRecoveryContext = !row.checkpointType && !row.resumeTargetJson?.trim();
    const restartRecoveryFailure = row.lastError?.includes("恢复失败") ?? false;

    return hasRecoveredProductionAssets && (missingRecoveryContext || restartRecoveryFailure);
  }

  async findDisplayAutoDirectorTaskByNovelId(novelId: string) {
    const activeRow = await this.findActiveTaskByNovelAndLane(novelId, "auto_director");
    if (activeRow) {
      return activeRow;
    }
    const latestRow = await this.findLatestVisibleTaskByNovelId(novelId, "auto_director");
    if (!latestRow) {
      return null;
    }
    if (await this.shouldHideStaleAutoDirectorTask(novelId, latestRow)) {
      return null;
    }
    return latestRow;
  }
