  ...
  render: (input) => {
    ...
    return [
      new SystemMessage([
        ...
        "",
        "【字段约束】",
        "1. chapterOrder 必须严格位于请求的区间内（从 startOrder 到 targetChapters），且按顺序连续递增。",
        "2. 不要生成区间之外的任何章节。",
        "3. beatType 必须准确表达该章的主要节拍功能，例如开局建立、冲突升级、信息揭示、关系变化、局面反转、高潮兑现、尾部钩子等。",
        "4. title 必须像真实可用的节拍标题，清晰体现该章核心推进，不要写成空泛标签。",
        "5. content 必须写清本章具体推进了什么、改变了什么、它在整体节奏中的作用是什么。",
        "6. status 必须用于表示该 beat 当前所处状态，保持全数组语义一致，不得乱用。",
        "",
        "【核心约束】",
        "1. 必须严格承接小说简介、世界上下文与作品圣经，尤其是要考虑 startOrder 之前已有的剧情基础。",
        ...
        "",
        "【质量要求】",
        "1. 如果从故事中段（startOrder > 1）开始生成，必须确保剧情衔接自然，不要突兀转向。",
        "2. 前几章必须快速建立主局面、主冲突或主卖点，避免迟迟不进入故事。",
        "3. 中段必须不断引入新变量、新压力、新选择或新后果，避免线性重复加码。",
        "4. 后段必须体现阶段性回报、局势收束或更大悬念，而不是平推结束。",
        "5. content 要强调“本章为什么值得存在”，而不是泛泛概括剧情。",
        "6. 参考资料只能借鉴技法、节奏、组织方式，不能照搬角色关系、剧情结构或桥段。",
        "",
        "【生成原则】",
      ...
      new HumanMessage([
        ...
        "",
        `【生成区间】第 ${input.startOrder ?? 1} 章 到 第 ${input.targetChapters} 章`,
        "请只生成此区间内的剧情节拍。",
        referenceBlock,
        "",
