  ...
  return {
    ...
    render: (input, context) => [
      new SystemMessage([
        ...
        "",
        "【硬性要求】",
        `recommendedVolumeCount 必须等于 ${input.suggestedVolumeCount}，且等于 volumes.length。`,
        ...
        "5. 如果用户只给了世界观或方向、没有明确完结章数，则 recommendedVolumeCount 只代表当前滚动规划窗口，不代表整本书最终章数。",
        "6. 如果用户给了预计章节数，也把它视为当前结构预算：前期可硬规划，中后期必须保留复盘和再规划空间，不要把 300 章以后每个阶段都写死。",
        "",
        "【规划原则】",
        ...
        "6. 长篇连载采用滚动规划：锁定当前卷和下一卷入口，后续只保留阶段方向、升级空间和不可踩红线；章节执行后允许根据实际进度重新生成后续卷/章。",
        "",
        "【重点判断项】",
        ...
        "",
        "【输出 JSON 字段合同】",
        "顶层必须是一个对象，并且只能围绕以下字段组织：",
        "{",
        '  "recommendedVolumeCount": 1,',
        '  "hardPlannedVolumeCount": 1,',
        '  "readerRewardLadder": "整书读者回报梯度",',
        '  "escalationLadder": "整书升级梯度",',
        '  "midpointShift": "中盘转向",',
        '  "notes": "分卷策略备注",',
        '  "volumes": [',
        "    {",
        ...
        '      "planningMode": "hard",',
        '      "roleLabel": "本卷阶段定位",',
        '      "coreReward": "本卷最核心的读者回报",',
        '      "escalationFocus": "本卷最主要的升级焦点",',
        ...
        "  ]",
        "}",
        "volumes 必须是数组，uncertainties 也必须是数组。",
        "uncertaintyLevel / level 只能是 low、medium、high。",
        "这个阶段不要输出卷骨架字段；不要写 summary、openingHook、mainPromise、primaryPressureSource、coreSellingPoint、escalationMode、protagonistChange、midVolumeRisk、climax、payoffType、nextVolumeHook、chapters。",
      ].join("\n")),
      new HumanMessage([
        "请基于以下上下文，输出整本书的分卷策略。",
        "",
        "【输出要求】",
        "- 只输出严格 JSON",
        `- recommendedVolumeCount 必须等于 ${input.suggestedVolumeCount}`,
        "- volumes.length 必须等于 recommendedVolumeCount",
        "- 前 hardPlannedVolumeCount 卷必须为 hard，后续卷必须为 soft",
        "- 优先保证前期抓力、中期续航与后期可调度性",
        "- 每个 volume 项只写 sortOrder / planningMode / roleLabel / coreReward / escalationFocus / uncertaintyLevel",
        "- 不要混入卷骨架字段，例如 summary / openingHook / mainPromise / chapters",
        "",
        "【规划上下文】",
